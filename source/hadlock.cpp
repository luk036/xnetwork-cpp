...
template auto biconnected_components<xnetwork::SimpleGraph>(
    const xnetwork::SimpleGraph& G)
    -> std::vector<py::set<typename xnetwork::SimpleGraph::node_t>>;

template auto exact_mwpm<uint32_t>(
    const std::vector<int>&, const std::vector<std::vector<int>>&)
    -> std::vector<std::pair<int, int>>;

template auto dijkstra<uint32_t>(
    const std::vector<std::vector<DualEdge<uint32_t>>>&, int)
    -> std::pair<std::vector<int>, std::vector<int>>;

template auto greedy_mwpm<uint32_t>(
    const std::vector<int>&, const std::vector<std::vector<int>>&)
    -> std::vector<std::pair<int, int>>;
