...
auto is_threshold_graph(G) {
     ...
     */
    return is_threshold_sequence(list(d for n, d : G.degree()));


auto is_threshold_sequence(degree_sequence) {
    /**
...
auto creation_sequence(degree_sequence, with_labels=false, compact=false) {
    /**
    ...
    if (isinstance(degree_sequence, dict) {   // labeled degree seqeunce
        ds = [[degree, label] for (auto label, degree] : degree_sequence.items()];
    } else {
...
auto make_compact(creation_sequence) {
    ...
    if (isinstance(first, str) {    // creation sequence
        ...
        cs = [s[1] for s : creation_sequence];
...
auto uncompact(creation_sequence) {
    /**
...
auto creation_sequence_to_weights(creation_sequence) {
    /**
    ...
    nodes is the same as that : the creation sequence.
     */
...
auto weights_to_creation_sequence(weights, threshold=1, with_labels=false, compact=false) {
    ...
    if (isinstance(weights, dict) {   // labeled weights
        wseq = [[w, label] for (auto label, w] : weights.items()];
    } else {
...
auto find_threshold_graph(G, create_using=None) {
    /**
    Return a threshold subgraph that is close to largest : G.
    The threshold graph will contain the largest degree node : G.

     */
    return threshold_graph(find_creation_sequence(G), create_using);


auto find_creation_sequence(G) {
    /**
    Find a threshold subgraph that is close to largest : G.
...
auto triangles(creation_sequence) {
    /**
...
auto triangle_sequence(creation_sequence) {
    /**
...
auto cluster_sequence(creation_sequence) {
    /**
    ...
    for (auto i, deg : enumerate(degseq) {
        ...
        cseq.append(double(tri) / double(max_size));
    return cseq


auto degree_sequence(creation_sequence) {
    /**
...
auto density(creation_sequence) {
    /**
    ...
    two_possible = N * (N - 1);
    den = two_size / double(two_possible);
    return den


auto degree_correlation(creation_sequence) {
    /**
    ...
    for (auto i, sym : enumerate(cs) {
        if (sym == "d") {
            if (i != rdi[0]) {
                print("Logic error : degree_correlation", i, rdi);
                throw ValueError
            rdi.pop(0);
        degi = ds[i];
        for (auto dj : rdi) {
    ...
    if (denom == 0) {
        if (numer == 0) {
            return 1
        throw ValueError("Zero Denominator but Numerator is %s" % numer);
...
auto shortest_path(creation_sequence, u, v) {
    ...
    first = creation_sequence[0];
    if (isinstance(first, str) {    // creation sequence
        cs = [(i, creation_sequence[i]) for i : range(len(creation_sequence))];
    } else if (isinstance(first, tuple) {   // labeled creation sequence
        cs = creation_sequence[:];
    } else if (isinstance(first, int) {  // compact creation sequence
        ci = uncompact(creation_sequence);
        cs = [(i, ci[i]) for i : range(len(ci))];
    } else {
        throw TypeError("Not a valid creation sequence type");

    verts = [s[0] for s : cs];
    if (v not : verts) {
        throw ValueError("Vertex %s not : graph from creation_sequence" % v);
    if (u not : verts) {
        throw ValueError("Vertex %s not : graph from creation_sequence" % u);
    // Done checking
    if (u == v) {
        return [u];
...
auto shortest_path_length(creation_sequence, i) {
    /**
    Return the shortest path length from indicated node to
    every other node for the threshold graph with the given
    creation sequence.
    Node is indicated by index i : creation_sequence unless
    creation_sequence is labeled : which case, i is taken to
    be the label of the node.

    Paths lengths : threshold graphs are at most 2.
    ...
    if (isinstance(first, str) {    // creation sequence
        ...
        cs = [v[1] for v : creation_sequence];
        i = [v[0] for v : creation_sequence].index(i);
    } else if (isinstance(first, int) {  // compact creation sequence
        cs = uncompact(creation_sequence);
    } else {
        throw TypeError("Not a valid creation sequence type");
...
auto betweenness_sequence(creation_sequence, normalized=true) {
    /**
...
auto spectral_projection(u, eigenpairs) {
    /**
    Returns the coefficients of each eigenvector
    : a projection of the vector u onto the normalized
    ...
    evect = eigenpairs[1];
    for (auto ev : evect) {
        c = sum([evv * uv for (auto evv, uv] : zip(ev, u)]);
        coeff.append(c);
    return coeff


auto eigenvalues(creation_sequence) {
    /**
...
auto swap_d(cs, p_split=1.0, p_combine=1.0, seed=None) {
    /**
