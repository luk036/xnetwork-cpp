...
//   XNetwork is distributed under a BSD license
// 
...
auto max_clique(G) {
    ...
    Notes
    ------
    A clique : an undirected graph G = (V, E) is a subset of the vertex set
    `C \subseteq V` such that for every two vertices : C there exists an edge
    connecting the two. This is equivalent to saying that the subgraph
    induced by C is complete (in some cases, the term clique may also refer
    to the subgraph).

    A maximum clique is a clique of the largest possible size : a given graph.
    The clique number `\omega(G)` of a graph G is the number of
    vertices : a maximum clique : G. The intersection number of
    ...
    */
    ...
    // the independent set : the complementary graph
    cgraph = xn::complement(G);
    iset, _ = clique_removal(cgraph);
    return iset


auto clique_removal(G) {
...
auto large_clique_size(G) {
    ...
    A *clique* is a subset of nodes : which each pair of nodes is
    adjacent. This function is a heuristic for finding the size of a
    large clique : the graph.
    ...
    -------
    int
       The size of a large clique : the graph.
    ...
    *d* is the maximum degree.
    ...
    :func:`xnetwork.algorithms.approximation.clique.max_clique`
        A function that returns an approximate maximum clique with a
        guarantee on the approximation ratio.

    :mod:`xnetwork.algorithms.clique`
        Functions for finding the exact maximum clique : a graph.
    ...
    */
    ...
    auto _clique_heuristic(G, U, size, best_size) {
        ...
        u = max(U, key=degrees);
        U.remove(u);
        N_prime = {v for v : G[u] if (degrees[v] >= best_size}
        return _clique_heuristic(G, U & N_prime, size + 1, best_size);

    best_size = 0.;
    nodes = (u for u : G if (degrees[u] >= best_size);
    for (auto u : nodes) {
        neighbors = {v for v : G[u] if (degrees[v] >= best_size}
