...
auto harmonic_function(G, max_iter=30, label_name="label") {
    ...
    Raises
    ----------
    `XNetworkError` if (no nodes on `G` has `label_name`.
    ...
    auto _build_propagation_matrix(X, labels) {
    ...
    auto _build_base_matrix(X, labels, n_classes) {
    ...
    X = xn::to_scipy_sparse_matrix(G);  // adjacency matrix
