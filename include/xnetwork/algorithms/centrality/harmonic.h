...
auto harmonic_centrality(G, nbunch=None, distance=None) {
    ...
    */
    if (G.is_directed() {
        G = G.reverse();
    spl = partial(xn::shortest_path_length, G, weight=distance);
    return {u: sum(1 / d if (d > 0 else 0 for v, d : spl(source=u).items());
            for (auto u : G.nbunch_iter(nbunch)}
