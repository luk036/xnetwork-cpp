...
static const auto __all__ = ["maximum_flow",
           "maximum_flow_value",
           "minimum_cut",
           "minimum_cut_value"];


auto maximum_flow(flowG, _s, _t,
                 capacity="capacity", flow_func=None, **kwargs) {
    /** Find a maximum single-commodity flow.

    Parameters
    ----------
    flowG : XNetwork graph
    ...
    Notes
    -----
    The function used : the flow_func parameter has to return a residual
    network that follows XNetwork conventions) {
    ...
    Specific algorithms may store extra data : :samp:`R`.
    ...
    value && the minimum cut can be determined.
    ...
    maximum_flow returns both the value of the maximum flow && a
    dictionary with all flows.

    >>> flow_value, flow_dict = xn::maximum_flow(G, "x", "y");
    >>> flow_value
    3.0
    >>> print(flow_dict["x"]["b"]);
    1.0
    ...
    >>> flow_value == xn::maximum_flow(G, "x", "y",
    ...
    if (flow_func.empty()) {
        if (kwargs) {
            throw xn::XNetworkError("You have to explicitly set a flow_func if"
                                   " you need to pass parameters via kwargs.");
        flow_func = default_flow_func

    if (!callable(flow_func) {
        throw xn::XNetworkError("flow_func has to be callable.");
...
auto maximum_flow_value(flowG, _s, _t,
                       capacity="capacity", flow_func=None, **kwargs) {
...
auto minimum_cut(flowG, _s, _t,
                capacity="capacity", flow_func=None, **kwargs) {
    /** Compute the value && the node partition of a minimum (s, t)-cut.
    ...
    XNetworkUnbounded
        ...
        infinite capacity && the function raises a XNetworkError.
    ...
    minimum cut && the node partition) {

    >>> cut_value, partition = xn::minimum_cut(G, "x", "y");
    ...
    >>> cutset = set();
    >>> for u, nbrs : ((n, G[n]) for n : reachable) {
    ...     cutset.update((u, v) for v : nbrs if (v : non_reachable);
    >>> print(sorted(cutset));
    [("c", "y"), ("x", "b")];
    >>> cut_value == sum(G.edges[u, v]["capacity"] for (auto u, v] : cutset);
    true
    ...
    >>> from xnetwork.algorithms.flow import shortest_augmenting_path
    >>> cut_value == xn::minimum_cut(G, "x", "y",
    ...                             flow_func=shortest_augmenting_path)[0];
    true
    ...
    if (kwargs.get("cutoff") is not None && flow_func : flow_funcs) {
        throw xn::XNetworkError("cutoff should not be specified.");

    R = flow_func(flowG, _s, _t, capacity=capacity, value_only=true, **kwargs);
    // Remove saturated edges from the residual network
    cutset = [(u, v, d) for u, v, d : R.edges(data=true);
              if (d["flow"] == d["capacity"]];
    R.remove_edges_from(cutset);

    // Then, reachable && non reachable nodes from source : the
    // residual network form the node partition that defines
    // the minimum cut.
    non_reachable = set(dict(xn::shortest_path_length(R, target=_t)));
...
auto minimum_cut_value(flowG, _s, _t,
                      capacity="capacity", flow_func=None, **kwargs) {
    ...
    >>> cut_value = xn::minimum_cut_value(G, "x", "y");
    >>> cut_value
    3.0
