...
auto voronoi_cells(G, center_nodes, weight="weight") {
    ...
    *v* that are closer to *c* than to any other center node : *C* with
    ...
    as defined : [1]_, : which distance is measured from the center
    ...
    -------
    dictionary
        A mapping from center node to set of all nodes : the graph
    ...
    take the collection of all values : the returned dictionary:) {

        >>> G = xn::path_graph(6);
        >>> center_nodes = {0, 3}
        >>> cells = xn::voronoi_cells(G, center_nodes);
    ...
    paths = xn::multi_source_dijkstra_path(G, center_nodes, weight=weight);
    // Determine the center node from which the shortest path originates.
    nearest = {v: p[0] for v, p : paths.items()}
