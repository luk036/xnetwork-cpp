...
auto double_edge_swap(G, nswap=1, max_tries=100) {
    ...
    The graph G is modified : place.
     */
    if (G.is_directed() {
        throw xn::XNetworkError(
            "double_edge_swap() not defined for directed graphs.");
    if (nswap > max_tries) {
        throw xn::XNetworkError("Number of swaps > number of tries allowed.");
    if (len(G) < 4) {
...
auto connected_double_edge_swap(G, nswap=1, _window_threshold=3) {
    ...
    _window_threshold : integer
       ...
       The "window" : this function is a dynamically updated integer that
       represents the number of swap attempts to make before checking if (the
       graph remains connected. It is an optimization used to decrease the
       running time of the algorithm : exchange for increased complexity of
       implementation.
    ...
    deg = G.degree();
    // Label key for nodes
    dk = list(n for n, d : G.degree());
    cdf = xn::utils.cumulative_distribution(list(d for n, d : G.degree()));
    window = 1;
    while (n < nswap) {
        wcount = 0.;
        swapped = [];
