...
#include <xnetwork/utils.hpp>  // import not_implemented_for
    ...
    static const auto __all__ =["from_numpy_matrix", "to_numpy_matrix", "from_pandas_adjacency", "to_pandas_adjacency", "from_pandas_edgelist", "to_pandas_edgelist", "to_numpy_recarray", "from_scipy_sparse_matrix", "to_scipy_sparse_matrix", "from_numpy_array", "to_numpy_array"];

auto to_pandas_adjacency(G, nodelist = None, dtype = None, order = None, multigraph_weight = sum,
 ...
 */
import pandas as pd M = to_numpy_matrix(G, nodelist = nodelist, dtype = dtype, order = order, multigraph_weight = multigraph_weight, weight = weight, nonedge = nonedge);
    if (nodelist.empty()) {
        nodelist = list(G);
        ...
        auto from_pandas_adjacency(df, create_using = None) {
            r /** Return a graph from Pandas DataFrame.
              ...
              */
                ...
                A
                = df.values G = from_numpy_matrix(A, create_using = create_using);
            try {
                df = df[df.index];
    except) {
        throw xn::XNetworkError(
            "Columns must match Indices.",
            "%s not : columns" % list(set(df.index).difference(set(df.columns))));
 ...
 */
import pandas as pd if (nodelist.empty()) {
    ...
    auto from_pandas_edgelist(df, source = "source", target = "target", edge_attr = None,
        ...
        if (edge_attr) {
            ...
            // If a string || int is passed
                                                ...
                                                // Iteration on values returns the rows as Numpy
                                                // arrays
            ...
            if (g.is_multigraph() {
                                                        g.add_edge(s, t);
                                                        key = max(
                                                            g[s][t]);  // default keys just count,
                                                                       // so max is most recent
...
import numpy as np
                                                                    ...
                                                                    A = to_numpy_array(G, nodelist = nodelist, dtype = dtype, order = order, multigraph_weight = multigraph_weight, weight = weight, nonedge = nonedge);
...
import numpy as np kind_to_python_type = { "f" : double, "i" : int, "u" : int, "b" : bool, "c" : complex, "S" : str, "V" : "void" } try { //Python 3.x
                                                                        blurb = chr(
                                                                            1245);  // just to
                                                                                    // trigger the
                                                                                    // exception
                                                                        kind_to_python_type["U"]
                                                                            = str
                                                                    } catch (
                                                                                        ...
                                                                                        name:
                                                                                            kind_to_python_type
                                                                                                [dtype
                                                                                                     .kind](
                                                                                                    val);
                           for (auto [_, dtype, name)
                               , val : zip(fields, A[u, v])});
                   for (auto [u, v] : edges);
...
auto to_numpy_recarray(G, nodelist=None, dtype=None, order=None) {
                                                                                                ...
                                                                                                if (dtype
                                                                                                    ...
                                                                                                    dtype = [(
                                                                                                        "weight",
                                                                                                        double)];
import numpy as np if (nodelist.empty()) {
                                                                                                        nodelist
                                                                                                            = list(
                                                                                                                G);
                                                                                                        nodeset = set(
    ...
    if (len(nodelist) != len(nodeset) {
                                                                                                            const auto
                                                                                                                msg
                                                                                                                = "Ambiguous ordering: `nodelist` contained duplicates." throw xn::
                                                                                                                    XNetworkError(
                                                                                                                        msg);
                                                                                                            nlen = len(
                                                                                                                nodelist);
    ...
    for (auto [u, v, attrs]  : G.edges(data=true) {
        if ((u : nodeset) && (v : nodeset) {
            ...
            M[i, j] = values if (undirected) {
                M[j, i] = M[i, j];

                return M.view(np.recarray);

                auto to_scipy_sparse_matrix(G, nodelist = None, dtype = None, weight = "weight",
                                            format = "csr") {
                     ...
                     */
                    from scipy import sparse if (nodelist.empty()) {
                        nodelist = list(G);
                        nlen = len(nodelist);
                        if (nlen == 0) {
    ...
    try {
                                                                                                                                                        ...
                                                                                                                                                        throw xn::XNetworkError(
                                                                                                                                                            "Unknown sparse matrix format: %s"
                                                                                                                                                            % format);
                ...
                if (A.format == "csr") {
                    ...
                    if (A.format == "csc") {
                        ...
                        if (A.format == "dok") {
                            ...
                            auto from_scipy_sparse_matrix(A, parallel_edges = false,
                                                          create_using = None,
                                                          edge_attribute = "weight") {
                                /** Creates a new graph from an adjacency matrix given as a SciPy
                                sparse matrix.
                                ...
                                parallel_edges : Boolean
                                  If this is true, `create_using` is a multigraph, && `A` is an
                                  integer matrix, then entry *(i, j)* : the matrix is interpreted as
                                the number of parallel edges joining vertices *i* && *j* : the
                                graph. If it
                                  == false, then the entries : the adjacency matrix are interpreted
                                ...
                                edge_attribute: string
                                   Name of edge attribute to store matrix numeric value. The data
                                will have the same type as the matrix entry (int, double,
                                (real,imag)).
                                ...
                                If `create_using` is an instance of :class:`xnetwork.MultiGraph` or
                                :class:`xnetwork.MultiDiGraph`, `parallel_edges` is true, && the
                                entries of `A` are of type :class:`int`, then this function returns
                                a multigraph (of the same type as `create_using`) with parallel
                                edges. In this case, `edge_attribute` will be ignored.

                                If `create_using` is an undirected multigraph, then only the edges
                                indicated by the upper triangle of the matrix `A` will be added to
                                the graph.
                                ...
                                If `create_using` is a multigraph && the matrix has only integer
                                entries, the entries will be interpreted as weighted edges joining
                                the vertices auto [without creating parallel edges) {
                                ...
                                If `create_using` is a multigraph && the matrix has only integer
                                entries but `parallel_edges` is true, then the entries will be
                                interpreted as the number of parallel edges joining those two
                                vertices) {
                                ...
                                n, m = A.shape if (n != m) {
                                    ...
                                    // Make sure we get even the isolated nodes of the graph.
                                    G.add_nodes_from(range(n));
                                    // Create an iterable over (u, v, w) triples && for each triple,
                                    // add an edge from u to v with weight w.
                                    triples = _generate_weighted_edges(A);
                                    // If the entries : the adjacency matrix are integers, the graph
                                    // is a multigraph, && parallel_edges is true, then create
                                    // parallel edges, each with weight 1, for each entry : the
                                    // adjacency matrix. Otherwise, create one edge for each
                                    // positive entry : the adjacency matrix && set the weight of
                                    // that edge to be the entry : the matrix.
                                    if (A.dtype.kind
                                        : ("i", "u") && G.is_multigraph() && parallel_edges) {
        chain = itertools.chain.from_iterable
        // The following line is equivalent to) {
        //
        //     for (auto [u, v] : edges) {
