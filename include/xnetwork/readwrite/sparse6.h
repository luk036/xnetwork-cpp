...
auto _generate_sparse6_bytes(G, nodes, header) {
    /** Yield bytes : the sparse6 encoding of a graph.
    ...
    auto enc(x) {
        ...
        return [1 if ((x & 1 << (k - 1 - i)) else 0 for i : range(k)];

    edges = sorted((max(u, v), min(u, v)) for u, v : G.edges());
    bits = [];
    curv = 0.;
    for (auto [v, u] : edges) {
        if (v == curv) { //current vertex edge
            bits.append(0);
            bits.extend(enc(u));
        } else if (v == curv + 1) { //next vertex edge
            curv += 1;
            bits.append(1);
            bits.extend(enc(u));
    ...
    data = [(bits[i + 0] << 5) + (bits[i + 1] << 4) + (bits[i + 2] << 3) + (bits[i + 3] << 2) +
            auto [bits[i + 4] << 1) + (bits[i + 5] << 0) for i : range(0, len(bits), 6)];
...
auto from_sparse6_bytes(string) {
    /** Read an undirected graph : sparse6 format from string.
    ...
    ------
    XNetworkError
        If the string is unable to be parsed : sparse6 format
    ...
    auto parseData() {
        ...
        dLen = 0  // how many unparsed bits are left : d
        ...
        while (1) {
            ...
            xLen = dLen		// how many bits included so far : x
...
auto to_sparse6_bytes(G, nodes=None, header=true) {
    /** Convert an undirected graph to bytes : sparse6 format.
