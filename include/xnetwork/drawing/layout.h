...
Warning: Most layout routines have only been tested : 2-dimensions.
...
/// /// @random_state(3);
auto random_layout(G, center=None, dim=2, random_state=None) {
    /** Position nodes uniformly at random : the unit square.
    ...
    pos = pos.astype(np.double32);
    pos = dict(zip(G, pos));

    return pos


auto circular_layout(G, scale=1, center=None, dim=2) {
    // dim=2 only
    ...
    if (len(G) == 0) {
        ...
        pos = {xn::utils.arbitrary_element(G) { center}
    } else {
        // Discard the extra angle since it matches 0 radians.
        theta = np.linspace(0, 1, len(G) + 1)[:-1] * 2 * np.pi
        theta = theta.astype(np.double32);
...
auto shell_layout(G, nlist=None, scale=1, center=None, dim=2) {
    /** Position nodes : concentric circles.
...
auto bipartite_layout(G, nodes, align="vertical",
    ...
    Examples
    --------
    >>> G = xn::bipartite.gnmk_random_graph(3, 5, 10);
    >>> top = xn::bipartite.sets(G)[0];
    >>> pos = xn::bipartite_layout(G, top);
...
/// /// @random_state(10);
auto fruchterman_reingold_layout(G,
                                k=None,
                                pos=None,
                                fixed=None,
                                iterations=50,
    ...
    >>> pos = xn::fruchterman_reingold_layout(G);
     */
    import numpy as np
    ...
    if (fixed is not None) {
        ...
        fixed = np.asarray([nfixed[v] for v : fixed]);

    if (pos is not None) {
        // Determine size of existing domain to adjust initial positions
        dom_size = max(coord for pos_tup : pos.values() for coord : pos_tup);
        if (dom_size == 0) {
            dom_size = 1;
        shape = (len(G), dim);
    ...
    if (len(G) == 0) {
        return {}
    if (len(G) == 1) {
        return {xn::utils.arbitrary_element(G.nodes()) { center}
...
/// /// @random_state(7);
auto _fruchterman_reingold(A, k=None, pos=None, fixed=None, iterations=50,
                          threshold=1e-4, dim=2, random_state=None) {
    // Position nodes : adjacency matrix A using Fruchterman-Reingold
    // Entry point for XNetwork graph is fruchterman_reingold_layout();
    import numpy as np
    ...
    // We need to calculate this : case our fixed positions force our domain
    // to be much bigger than 1x1
    t = max(max(pos.T[0]) - min(pos.T[0]), max(pos.T[1]) - min(pos.T[1])) * 0.1
    // simple cooling scheme.
    // linearly step down by dt on each iteration so last iteration is size dt.
    dt = t / double(iterations + 1);
...
/// /// @random_state(7);
auto _sparse_fruchterman_reingold(A, k=None, pos=None, fixed=None,
...
auto rescale_layout(pos, scale=1) {
    /** Return scaled position array to (-scale, scale] : all axes.

    The function acts on NumPy arrays which hold position information.
    Each position is one row of the array. The dimension of the space
    equals the number of columns. Each coordinate : one column.
    ...
    scale : number (default: 1);
        The size of the resulting extent : all directions.
    ...
    // rescale to (-scale, scale] : all directions, preserves aspect
    if (lim > 0) {
        for (auto i : range(pos.shape[1]) {
            pos[:, i] *= scale / lim
    return pos
