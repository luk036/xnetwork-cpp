...
auto prim_mst(size_t n, WeightFunc&& weight) -> std::vector<std::pair<Node, Node>>
{
    ...
    for (size_t count = 0; count < n; ++count)
    {
        ...
        if (best == INF)
            break; // isolated vertex (shouldn't happen for complete graphs)
...
auto find_odd_degree_nodes(const std::vector<std::pair<Node, Node>>& mst_edges,
                           size_t n) -> std::vector<Node>
{
    ...
    for (Node v = Node{0}; v < static_cast<Node>(n); ++v)
    {
        if (deg[static_cast<size_t>(v)] % 2 != 0)
            odd.push_back(v);
    }
    return odd;
}
 ...
 */
template <typename Node, typename WeightFunc>
auto greedy_min_weight_matching(const std::vector<Node>& odd_nodes,
                                WeightFunc&& weight)
 ...
 */
template <typename Node>
auto build_multigraph(size_t n,
    ...
    -> std::vector<std::multiset<Node>>
{
    ...
    for (const auto& [u, v] : mst_edges)
        add(u, v);
    for (const auto& [u, v] : matching_edges)
        add(u, v);
    return adj;
}
...
auto christofides_tsp(const Graph& G, WeightFunc&& weight)
