...
auto partial_duplication_graph(N, n, p, q, seed=None) {
    ...
    ----------
    N : int
        The total number of nodes : the final graph.

    n : int
        The number of nodes : the initial clique.
    ...
    for (auto new_node : range(n, N) {
        ...
        for (auto neighbor_node : list(xn::all_neighbors(G, src_node)) {
...
auto duplication_divergence_graph(n, p, seed=None) {
    ...
    ----------
    n : int
        The desired number of nodes : the graph.
    p : double
    ...
    while (i < n) {
        ...
        flag  = false;
        for (auto nbr : G.neighbors(random_node) {
            if (random.random() < p) {
                // Link retention step.
                G.add_edge(i, nbr);
                flag  = true;
