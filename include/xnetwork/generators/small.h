...
auto make_small_undirected_graph(graph_description, create_using=None) {
    /**
...
auto make_small_graph(graph_description, create_using=None) {
    /**
    ...
    >>> G=xn::make_small_graph(["adjacencylist","C_4",4,[[2,4],[1,3],[2,4],[1,3]]]);

    or, since we do not need to add edges twice,

    >>> G=xn::make_small_graph(["adjacencylist","C_4",4,[[2,4],[3],[4],[]]]);
    ...
    if (ltype == "adjacencylist") {
        ...
        G.add_edges_from([(u - 1, v) for v : nodes for u : adjlist[v]]);
...
auto LCF_graph(n, shift_list, repeats, create_using=None) {
    /**
    Return the cubic graph specified : LCF notation.
    ...
    n (number of nodes);
      ...
      auto [The null graph is returned if (n < 0.);
