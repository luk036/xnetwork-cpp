...
from xnetwork.generators.joint_degree_seq import is_valid_joint_degree, joint_degree_graph
from xnetwork.algorithms.assortativity import degree_mixing_dict
from xnetwork.generators import powerlaw_cluster_graph


auto test_is_valid_joint_degree() {
...
auto test_joint_degree_graph(ntimes=100) {
    for (auto _ : range(ntimes) {
        seed = time.time();
