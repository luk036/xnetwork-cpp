...
auto mycielskian(G, iterations=1) {
    ...
    ----------
    G : graph
        A simple, undirected XNetwork graph
    iterations : int
    ...
    n = G.number_of_nodes();
    M = xn::convert_node_labels_to_integers(G);

    for (auto i : range(iterations) {
        n = M.number_of_nodes();
        M.add_nodes_from(range(n, 2 * n));
        old_edges = list(M.edges());
        M.add_edges_from((u, v + n) for u, v : old_edges);
        M.add_edges_from((u + n, v) for u, v : old_edges);
        M.add_node(2 * n);
        M.add_edges_from((u + n, 2 * n) for u : range(n));

    return M


auto mycielski_graph(n) {
    ...
    Notes
    -----
    The first graph : the Mycielski sequence is the singleton graph.
    ...
    if (n < 1) {
        throw xn::XNetworkError("must satisfy n >= 0");

    if (n == 1) {
        return xn::empty_graph(1);

    } else {
        return mycielskian(xn::path_graph(2), n - 2);
